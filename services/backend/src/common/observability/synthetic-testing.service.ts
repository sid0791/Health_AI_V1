...
export class SyntheticTestingService {
  ...
  async runTest(testId: string): Promise<TestResult> {
    ...
    try {
      ...
      this.logger.log(`Test ${test.name} completed: ${result.success ? 'PASS' : 'FAIL'} (${result.responseTime}ms)`);
  ...
  getHealthDashboard(): {
    ...
    const overview = {
      ...
      successRate: allSummaries.length > 0 
        ? allSummaries.reduce((sum, s) => sum + s.successRate, 0) / allSummaries.length
        : 0,
      avgResponseTime: allSummaries.length > 0
    ...
    for (const [testId, results] of this.testResults.entries()) {
      ...
      const recentFailed = results.filter(r => 
        !r.success && 
   ...
   */
  private async runExternalServiceTest(test: SyntheticTest, startTime: number): Promise<TestResult> {
