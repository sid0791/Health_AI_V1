...
export class Level1RateLimitService {
  ...
  async checkRateLimit(userId: string): Promise<RateLimitInfo> {
    ...
    const minuteLimit = await this.checkTimeBasedLimit(userId, 'minute', this.RATE_LIMITS.perMinute, 60);
    if (minuteLimit.blocked) {
      return minuteLimit;
    }

    // Check per-hour limit (15 requests)
    const hourLimit = await this.checkTimeBasedLimit(userId, 'hour', this.RATE_LIMITS.perHour, 3600);
  ...
  async recordRequest(userId: string): Promise<void> {
    ...
    const cooldownKey = `${this.CACHE_PREFIXES.cooldown}${userId}`;
    await this.cacheManager.set(cooldownKey, this.RATE_LIMITS.cooldownSeconds * 1000, this.RATE_LIMITS.cooldownSeconds);
    ...
    this.logger.debug(`Level 1 request recorded for user ${userId} with ${this.RATE_LIMITS.cooldownSeconds}s cooldown`);
  }
  ...
  async getRateLimitStatus(userId: string): Promise<{
    ...
    const perMinute = await this.checkTimeBasedLimit(userId, 'minute', this.RATE_LIMITS.perMinute, 60);
    ...
    const nextAllowedRequest = new Date(Date.now() + Math.max(
      cooldownRemaining,
  ...
  throwRateLimitException(userId: string, rateLimitInfo: RateLimitInfo): never {
    ...
    this.logger.warn(`Level 1 rate limit exceeded for user ${userId}: ${waitTimeSeconds}s wait required`);
  ...
  private async incrementCounter(userId: string, timeWindow: 'minute' | 'hour' | 'day', ttlSeconds: number): Promise<void> {
    const key = `${this.CACHE_PREFIXES[timeWindow]}${userId}`;
    const currentCount = (await this.cacheManager.get<number>(key)) || 0;
  ...
  private getWindowStart(timestamp: number, windowSeconds: number): number {
    if (windowSeconds === 60) { // minute window
      const date = new Date(timestamp);
      date.setSeconds(0, 0);
      return date.getTime();
    } else if (windowSeconds === 3600) { // hour window  
      const date = new Date(timestamp);
      date.setMinutes(0, 0, 0);
      return date.getTime();
    } else if (windowSeconds === 86400) { // day window
      const date = new Date(timestamp);
