...
export class AIMealGenerationService {
  ...
  private async calculateAccurateNutrition(recipes: CelebrityStyleRecipe[]): Promise<any> {
    ...
    for (let day = 1; day <= 7; day++) {
      ...
      for (const recipe of dayRecipes) {
        ...
        dayNutrition.calories += portionNutrition.nutritionPerServing.energy;
        dayNutrition.protein += portionNutrition.nutritionPerServing.protein;
        dayNutrition.carbs += portionNutrition.nutritionPerServing.carbohydrates;
        dayNutrition.fat += portionNutrition.nutritionPerServing.fat;
        dayNutrition.fiber += portionNutrition.nutritionPerServing.fiber;
        dayNutrition.avgGlycemicIndex += glycemicData.gl;
        dayNutrition.avgGlycemicLoad += glycemicData.gl;
      }
  ...
  private async analyzeNutritionalCompliance(
    ...
    const goalCompliance = {
      ...
      complianceScore: this.calculateComplianceScore(weeklyAverages, planPreferences),
    };
